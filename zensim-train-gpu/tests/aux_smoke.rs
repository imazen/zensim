...
use zensim_train_gpu::{
    GpuAnchorRows, GpuEquivPairs, GpuHparams, GpuRuntime, train_per_sample_alpha_head_gpu_with_aux,
};
...
fn build_synth_dataset(n_features: usize, n_rows: usize, seed: u64) -> (Vec<Vec<f64>>, Vec<f64>) {
...
fn aux_anchor_only_does_not_crash() {
    ...
    assert!(
        res.model.w1.iter().all(|v| v.is_finite()),
        "w1 NaN/Inf after anchor aux"
    );
