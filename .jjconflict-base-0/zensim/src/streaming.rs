...
use crate::blur::{
    box_blur_1pass_into, box_blur_v_from_copy, downscale_2x_inplace, fused_blur_h_mu,
...
fn process_strip_channel(
    ...
    if config.blur_passes == 1 {
        ...
        let store_mu =
            config.extended_features || config.compute_iw_features || dm_needs_edge || dm_needs_hf;
        if need_ssim {
        ...
        if need_activity {
            ...
            if do_ext && do_iw {
                ...
                let mse_m = build_mask_weight_and_mse(
                    activity_inner,
                    k,
                    inner_src,
                    inner_dst,
                ...
                // do_iw only
                let iw_out = &mut bufs.iw_weight[inner_off..inner_off + inner_n];
                let mse_i = build_iw_weight_and_mse(
                    activity_inner,
                    k_iw,
                    inner_src,
                    inner_dst,
