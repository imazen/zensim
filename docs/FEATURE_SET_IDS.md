...
| 9 | **⚠ CORRECTED 2026-09-05 — this is an ERA artifact of two stored INSTRUMENTS, not a property of the code.** The row read: *"The v1-372 `f0..155` is NOT the 944 fold's `f0..155`. Measured on 4,424 shared dial cells: 156 of 156 slots differ, max abs 1.0214."* That measurement compared two tables **built in different extractor eras**. In ONE process, at ONE commit, on the same pixels, they do not differ at all: **372 of 372 slots bit-identical at every one of 11 geometries**, at both SIMD tiers, through the public free function both v1-372 extractors call — option C (`56bbcda2`) closed the divergence. The row remains a true and useful warning about the INSTRUMENTS (do not column-mix tables across eras); read as a claim about the CODE it sends a reader hunting a divergence that no longer exists. Gate: `extractor_entry_is_bit_exact_to_the_fold_v1_block`. | original: `~/tmp/gaddrinst_RESUME.md` finding 3. Correction: `docs/FEATURE_DEFECTS_AUDIT_2026-09-05.md` §4.4, registered `feature-set-ids-row9-is-an-era-artifact-2026-09-05` |
