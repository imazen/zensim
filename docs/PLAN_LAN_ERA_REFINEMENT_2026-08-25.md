...
- zenav1-svt: **✓ loop + ✓ census CLOSED 2026-08-27** — `svtav1-target` crate (search `57805614` + judge-injected trial `508012c8`, tested through the real encoder) + phase-B census on the frozen HDR instrument (k2 17.64/1hit, k3 7.43/9hits, t80 1.20 — blind-seed cost quantified; zenav1-svt `52c8aba4`); seed arms register separately. zenav1-aom: ruled premature (its own gates first); gainmap: **✓ loop + ✓ census CLOSED 2026-08-27** — harness = zenmetrics `crates/zenmetrics-cli/examples/gainmap_zensim_census.rs` (in-process fleet arm + shelled sibling judge; registered pre-run `b61d2b0b`, closed `c537feef`): G-V PASS 27/27 both k, **0/27 hits (median |err| 71.5/70.4) — the arm is CEILING-BOUND** (today-judge per-scene ceilings −4.1..27.5, ALL < t70 under BOTH judge eras; two scenes negative). No seed arm can help this line; levers are ARM-side (multi-channel gain map, gm_scale=1, gm_quality>85, base tonemap) and register as arm waves. **ALL 7 ENCODER LINES NOW CLOSED OR RULED.** HDR instrument: ✓ refs frozen `7543b810`.
