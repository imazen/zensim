...
- Per-wave wall clock measured and falling: ✅ the diffmap rollup's wall-clock table (`f5878299`: 4.3/s bursts → 765/min sustained) + today's points: fix11 encode wave = **115 cells / ~2 min**; fix11 CPU-delta drains = 248 cells / ~4 min single-box; census waves each ≤1 session. ✅ GPU re-drain **COMPLETE 2026-08-27 14:21Z** — all six GPU runs live-gap ≤ 0; final topology 4 GPUs / 4 queues (i134, r7900x, r5600g provisioned-from-bare-metal, wsl-5070 via the cuda13 fix); mid-drain incidents root-caused+fixed at the data (11 bare-basename manifest jobs + 5 store-missing refs). Wall clock: the ~7.5k-cell GPU re-drain closed in ~6h including two box recoveries — each wave shorter than the last (criterion's falling-wall-clock line holds).
...
- hdrgrid: 🟡 encodes ✅ (incl. the 115-cell svt#11 backfill `a2e9b432`); diffmaps ✅ (193,800 finals, manifested+mirrored); cvvdp+zensim features ✅ re-drained clean; GPU metrics ✅ **COMPLETE + HARVESTED 2026-08-27** — post-drain writeback `harvest-2026-08-27/` (102,485 × 11 metric cols, miss_sha=0 miss_score=0; orientation ladders 99.1% PASS; `_MANIFEST.json` w/ build_commit+shas; Tower-mirrored, sha-verified). diffmap COMPLETE 193,800/193,804 (fan-4 recorded). **hdrgrid criterion-2 line: fully encoded, scored, featured, diffmapped, era-stamped, manifested, mirrored.**
- hdr_v3mix: ✅ @944 manifested w/ build_commit, orientation-gated PASS both splits, store+Tower mirrored (`7d4e2dcf`).
- **⚠ hdrgrid `zensim_score` is JUDGE-ERA-MIXED (found 2026-08-27 by the gainmap census cross-check, zenmetrics `c537feef`)**: identical blob scores 43.57 (ledger, wsl-c1/c2 08-07/08 era) vs 25.13 (today) — the `--hdr` zensim judge is a path dep on the live zensim tree, so each executor image bakes its build-date model; the 08-26/27 re-drain rows (image `9dffa5ca`) sit beside 08-07/08 bulk rows in the same run + `harvest-2026-08-26/scores.parquet`. ssim2/butteraugli/cvvdp unaffected (model-free); features regime-pinned separately. **Remedy EXECUTED 2026-08-27 (era-stamp + filter; the re-declare idea was WRONG — zenmetrics-9c verified JobIds carry no judge component, so re-declaring done ids re-scores ZERO cells by design):** `scripts/canonical_corpus/derive_hdrgrid_zensim_judge_era.py` derives era from the ledger (latest-wins; era windows asserted, nothing between) at file level (`zensim_judge_era.parquet` sha 6e9792f242b1995a…, 7,529 score-files: 4,149 B / 3,380 A — 3,065/3,420 (rendition,codec) keys era-MIXED, so file-level filtering is unusable) and at CELL level from the result blobs (`zensim_scores_by_judge_era.parquet` sha 063e5d70e689bea4…, keyed encode_sha: **96,900 cells = 87,632 era-B (90.4%, current judge 9dffa5ca) + 9,268 era-A**). Consumers (HDR-944 L1) train on the era-B slice. Full unification (tombstone-or-identity-tag re-score of the 9,268) = a ledger-design decision, USER-GATED, written up in both sessions' memories.
...
- imazen-26 audit: ✅ id half (`4d446c2d`/`e5974ca3`); the first dHash RUN half (`1bc58d4b`) was INVALIDATED 2026-08-27 (refs = the quarantined inspiration dir — user caught it; /mnt/v/imazen-26* never-use, canonical = the imazen/imazen-26 repo manifest + png-v3, o_NNNN = 4-digit id); ✅ CORRECTED run against the root source (2,157 manifest refs; chain byte-verified repo→picker-set→mirror): channel A synth-generator-sharing 71 ids, channel B cross-id dup-of-train 9 ids, eval exposure imazen26 8/74 · nonphoto-944 8/58 · nonphoto-372 16/128, and **measured realized inflation ≈0** (9 leader bakes ctrl-vs-clean, median Δ +0.0026, max |Δ| 0.0100 — audit md CORRECTED RESULTS). 🔶 EYE half = the user's (108 id-labeled montages_v3).
- hdrfeat944: **✅ COMPLETE (verified 2026-08-27 ~12:0xZ)** — all three arms done on the store (svt 3,410 + zenjxl 3,420 + gm 3,420, 0 failures; gm's paused flag = drained-run auto-pause). The 'other lane ❌' line was stale. HDR-944 L1 registered FROZEN (wave md) — executes mechanically at GPU-drain end.
...
- SOTA HDR: **✅ WAVE COMPLETE — SELECTED + PACKAGED HDR-944 CANDIDATE (2026-08-27, `bfab4951`+`6e77f5d8`)**: full-wave freeze_check over both arms × 3 seeds selects `HDR944_L1T1_s4005` (cvvdp-mix arm; 5/8 floors, sel_comp 0.8867; packaged bit-identical 180KB sha `d6203e9d…` with a REAL dial [11.4,87.0]); T2-s4004 = packaged runner-up (raw-UPIQ edge). Originally:  — L1-T2 breadth wave: UPIQ pooled ~0.70 (bar ≥0.55; L0 was 0.14–0.33), narwaria ~0.65 (bar >0; L0 zero), ALL 3 seeds pass; `HDR944_L1T2_s4004` freeze_check-SELECTED (E.4 chain, M3a measured 0.79) + PACKAGED (29,151 B, prune identity gate bit-identical, sha `614fa4fb…`). Caveats recorded (SDR-route pack anchor; sdr25 cross-route inversion; HDR-route dial anchor = registered future work). T1 (cvvdp-mix) at drain-end. + BHdr family (pure-Rust repro #68). 🔶 any FREEZE/default flip is the user's.
- Board/docs current: ✅ gauntlet REGENERATED 2026-08-27 with the six HDR-944 L1 candidates on the board (293 bakes; regen gates PASS: node-check + DOM-shim render + ECharts SSR + 598 registry badges); served at localhost:3300/zensim/reports/summer_gauntlet.html.
...
- Byte-identical + zenbench evidence: ✅ EXISTS PER KERNEL FAMILY in-repo — jxl `tier_isolation`/`kernel_tiers` + dispatch-parity batteries (`dct_idct_8x8_scalar_vs_dispatch_edge_battery`); zenavif `unpremul_tiers`/`tier_isolation` + the parallel lane's zenbench 4.0×@1920px (`09494c6`) + magetypes byte-identical-by-construction converter. ✅ consolidated re-RUN table LANDED: `benchmarks/perf_x86_tiers_rerun_2026-08-27.md` (jxl VarDCT 2.4×/modular 1.1× — the NEON record's mode split reproduced on x86; zenavif unpremul 3.3–4.0×, yuv420 1.4–1.6×; kernel_tiers documented aarch64-only; no target-cpu=native, RUSTFLAGS verified unset).

**6 BROWSER**
- ✅ SERVING: coefficient repo viewer (located + extended, never forked): canonical 7-codec pool + HDR-grid pool (`79ad39d`) + avifgen pool with zensim (`ccbbf65`+`c3eeacd`), render-gated 8/8; typed `flat-picker` builder registered for future pools (`7949671f`/`6c9e4942`). hdr_v3mix: **RULED N/A-BY-SHAPE 2026-08-27** — the @944 leg is a feature/target table (948 cols: 944 feats + ref_basename + 3 targets, ZERO encode refs/shas/urls — verified by schema read); there is no per-encode asset to browse, so criterion-6 does not apply to it (its model-side story lives on the gauntlet board). **CORRECTION 2026-08-27 (stale line)**: the hdrgrid pool ALREADY SERVES — `canonical-viewer-hdrgrid-2026-08-26` (102,485 rows, `?set=hdrgrid`, shipped by another session 08-26; DATA_PROVENANCE entry). ✅ **REFRESH EXECUTED 2026-08-27 (coefficient `fa1ef73`)**: committed viewer-base ETL (`scripts/etl_hdrgrid_viewer_base.py` — replaces the lost tmp-staged one), `score_zensim` = era-B slice only (9,454 era-A cells honestly NaN), fresh GPU metrics, rollup regenerated, **render gate 8/8**. Criterion-6: every encode set SERVES.
...
**The critical path to DONE (refreshed 2026-08-27):** every executable lane is closed or ruled; what remains is the user decision set on the board — D1 sharing-origin exclusion (measured ≈0 inflation, low-stakes), D2 HDR-944 freeze, D3 one-shot wiring (zenjpeg zq / jxl B3 / svt qp_start), D4 judge-era unification, D5 optional history rewrite — plus the other lane's zenav1-aom differential gates.
