...
| `D` (`zensim-d`) — generation-D FAST SDR profile, behind default-on `candidate-profiles`. Basic-only 372-declared-width linear bake; reads `f0..156` only, so a `feature-regime-v2` build can skip the whole `f156..372` pool pass (`ComputeSet::from_block_profile`, internal) — **added 2026-09-01** | `zensim/weights/d_sdr_add156_dense_dial_2026-08-31.bin` (internal `ADD156` / `ens-safesyn-only-raw-lasso`, spline-top-extended "arm A", 3,671 B, sha `4481c2d4…`) | `benchmarks/add156_ship_audit_2026-08-31.md`, `benchmarks/add156_d7_ood_guard_2026-08-31.pointer.md`, `benchmarks/era2_fast_profile_subset_2026-08-31.md`, `benchmarks/profile_d_and_published_speed_2026-09-01.md` |
