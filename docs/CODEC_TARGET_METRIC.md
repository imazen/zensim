...
| `D` (`zensim-d`) — generation-D FAST SDR profile, behind default-on `candidate-profiles`. Basic-only 372-declared-width linear bake; reads `f0..156` only, so a `feature-regime-v2` build can skip the whole `f156..372` pool pass (`ComputeSet::from_block_profile`, internal) — **added 2026-09-01** | `zensim/weights/d_sdr_add156_id100_negrich_dial_2026-09-05.bin` (internal `ADD156` / `ens-safesyn-only-raw-lasso` carrying the **id100+negrich dial**, 4,222 B, sha `921a8f67…` — **dial-era v2, flipped 2026-09-05 by user decision**; the forward pass is byte-identical to the era-1 `d_sdr_add156_dense_dial_2026-08-31.bin`, so rank/speed are unchanged and only absolute DIAL values move — identity 96.1157 → 100.0000, see `benchmarks/d_ship_flip_2026-09-05.md`) | `benchmarks/add156_ship_audit_2026-08-31.md`, `benchmarks/add156_d7_ood_guard_2026-08-31.pointer.md`, `benchmarks/era2_fast_profile_subset_2026-08-31.md`, `benchmarks/profile_d_and_published_speed_2026-09-01.md`, `benchmarks/d_ship_flip_2026-09-05.md` |
