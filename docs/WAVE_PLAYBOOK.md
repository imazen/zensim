...
| Complete ensemble pixel/spatial audit | `extract_features_372col --audit-ensemble <ordered paths> --audit-ensemble-weights <weights>` through `BakeScorer::ensemble` | Exact member/hash/weight identity, pixel/cache and attribution score parity; unsupported max/masked/IW terms remain explicit and block native steering |
...
| Spatial attribution / rectangle queries | `zensim/src/attribution.rs`, retained extraction in `feature_v2` | `BakeScorer::compute_with_ref_and_attribution` binds complete candidate scoring; signed score-unit density including candidate L8, explicit max/masked/IW gaps, scalar/feature parity and reusable binned sessions |
