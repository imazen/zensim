...
| Complete scalar ensemble cost | `zensim-bench/benches/ssim2_speed_bar.rs`, `ZEN_S2_ENSEMBLE` / `ZEN_S2_ENSEMBLE_WEIGHTS` | Canonical calibrated ensemble and member controls, explicit single-call samples, named hardware/threads, dispersion and strict contention admission; retained extrema/means are not p95 |
