...
//! # Idiomatic patterns
//!
//! ## The `scalar_op` must be a true mathematical reference
//!
//! The `scalar_op` closure is your **ground truth** — a pure f64 implementation
//! of the operation's mathematical definition. It must NOT be the `_scalar`
//! variant generated by `#[autoversion]`, because that variant is still compiled
//! code subject to auto-vectorization and floating-point reassociation. Use
//! textbook math in f64:
//!
//! ```rust,ignore
//! // GOOD: true mathematical reference
...
//! instead of dispatching internally. These are typically inner `#[arcane]`
//! functions called from within a dispatched context. To test them with the
//! oracle, summon the token in the closure:
//!
//! ```rust,ignore
...
//!     &input, w, h, 3,
//!     |buf, w, h| {
//!         // Summon the best available token for this CPU
//!         if let Some(t) = X64V3Token::summon() {
//!             process_v3(t, buf, w, h)
//!         } else {
//!             process_scalar(ScalarToken, buf, w, h)
//!         }
//!     },
...
//! ```
//!
//! Or more commonly, wrap the token-taking function in a dispatcher and test
//! through that. Oracle testing verifies the **output** is correct regardless
//! of which SIMD tier ran — use [SIMD consistency testing](crate::simd) to
//! verify **cross-tier equivalence** separately.
//!
