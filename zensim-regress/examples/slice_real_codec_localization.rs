...
fn read_pairs(n_per_cell: usize) -> Vec<(String, String, String, u32)> {
    ...
    type Pair = (String, String, u32);
    let mut by_cell: BTreeMap<CellKey, Vec<Pair>> = BTreeMap::new();
    ...
    for (i, line) in BufReader::new(file).lines().enumerate() {
        ...
        let Some(band) = bands.iter().find(|(lo, hi)| q >= *lo && q < *hi) else {
            continue;
        };
        let key = (codec.to_string(), *band);
        let entry = by_cell.entry(key).or_default();
        if entry.len() >= n_per_cell {
            continue;
        }
...
fn main() {
    ...
    eprintln!(
        "Loaded {} pairs across codec×band stratification",
        pairs.len()
    );
    ...
    for (codec, src_path, dec_path, q) in &pairs {
        let Some((src_raw, sw, sh)) = load_rgb(src_path) else {
            eprintln!("skip src {}", src_path);
            continue;
        };
        let Some((dst_raw, dw, dh)) = load_rgb(dec_path) else {
            eprintln!("skip dst {}", dec_path);
            continue;
        };
        if sw != dw || sh != dh {
            continue;
        ...
        let dm = z
            .compute_with_ref_and_diffmap(&pre_full, &dst_slice, opts)
            .unwrap();
        ...
        for (idx, &y0) in win_y_list.iter().enumerate() {
            ...
            let r = z
                .compute_with_ref_into(&pre_win, &win_dst_slice, &mut scratch)
                .unwrap();
        ...
        let canon_top3: std::collections::HashSet<_> =
            topk(&window_canon_neg, 3).into_iter().collect();
        ...
        if !s_canon_dm.is_nan() {
            summary_canon.push(s_canon_dm);
        }
        if !s_proxy_dm.is_nan() {
            summary_proxy.push(s_proxy_dm);
        }
        if canon_match {
            top1_canon_correct += 1;
        }
        if proxy_match {
            top1_proxy_correct += 1;
        }
    ...
    eprintln!(
        "\n# Per-pair SROCC vs ground-truth diffmap-aggregated damage (n={})",
        total
    );
    eprintln!(
        "# Option D (per-window canonical):  mean={:.4}  median={:.4}  p10={:.4}",
        mean(&summary_canon),
        median(&summary_canon),
        p10(&summary_canon)
    );
    eprintln!(
        "# Scale-0 proxy:                     mean={:.4}  median={:.4}  p10={:.4}",
        mean(&summary_proxy),
        median(&summary_proxy),
        p10(&summary_proxy)
    );
    ...
    eprintln!(
        "#   canonical: {:.2}/3",
        top3_canon_overlap_total as f64 / total as f64
    );
    eprintln!(
        "#   scale-0:   {:.2}/3",
        top3_proxy_overlap_total as f64 / total as f64
    );
