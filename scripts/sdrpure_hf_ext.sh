#!/usr/bin/env bash
# SPH1-M3a seed extension (registered in
# benchmarks/sdr_pure_retrain_wave_2026-08-28.md "SPH1-M3a seed extension").
...
HB=${SPH1B_HB:-$HOME/tmp/sph1b/heartbeat}
mkdir -p "$(dirname "$HB")"
...
for seed in 4006 4007 4008; do
    ...
    if [ ! -f "$OUT/$stem.bin" ]; then
        ...
        nice -n19 ionice -c3 "$REPO/target/release/zensim_mlp_train" "${ARGV[@]}" \
            ...
            --seed "$seed" --out "$OUT/$stem.bin" >> "$OUT/sph1_train.log" 2>&1 \
    ...
    if [ ! -f "$OUT/${stem}_packed.bin" ]; then
        ...
        nice -n19 ionice -c3 "$REPO/target/release/bake_dial_refit" pack \
            ...
            >> "$OUT/sph1_pack.log" 2>&1 || { say "PACK FAILED $stem"; FAILS=$((FAILS+1)); continue; }
    fi
    say "harvest ${stem}_packed"
    "$REPO/scripts/harvest_bakes.sh" --bake "$OUT/${stem}_packed.bin" --regime 944 \
        >> "$OUT/sph1_harvest.log" 2>&1 || { say "HARVEST FAILED ${stem}_packed"; FAILS=$((FAILS+1)); }
done
