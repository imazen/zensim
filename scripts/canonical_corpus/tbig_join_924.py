#!/usr/bin/env python3
"""Join the fleet's merged 924-feature sidecar (folded+append streaming regime) (fleet_blob_assemble_924.py fetch output, keyed `encode_sha` == canonical `encoded_filename`) to the
canonical-picker-2026-06-27 per-codec train/validate/test datasets, emitting
924-feature training views with ssim2/zensim targets.
...
Outputs, per codec-dataset and split:
  <out>/<dataset>/<split>_924.parquet
...
def main() -> int:
    ...
    feat_names = [f"f{i}" for i in range(924)]
        ...
        for (di, si), rows in buckets.items():
            ...
            if wkey not in writers:
                ...
                writers[wkey] = pq.ParquetWriter(
                    os.path.join(args.out, dset, f"{split}_924.parquet"),
                    out_schema,
                    compression="zstd",
                )
