...
HB=$HOME/tmp/w12u2sel/heartbeat
mkdir -p "$(dirname "$HB")"
...
echo -e "seed\tepoch\tm3a\tcid22\tvhfnl" > "$OUT/w12u2_selection.tsv"
for seed in 4031 4032 4033; do
  CK="$OUT/LSTAR2_s${seed}_ckpts"
  for ck in "$CK"/ckpt_epoch0{4,5,6,7,8}0.bin "$OUT/LSTAR2_s${seed}.bin"; do
    [ -f "$ck" ] || continue
    base=$(basename "$ck" .bin)
    ep=$(echo "$base" | grep -o '[0-9]*$' || echo final); [ "$base" = "LSTAR2_s${seed}" ] && ep=final
    ...
    [ -f "$pk" ] || nice -n19 ionice -c3 "$REPO/target/release/bake_dial_refit" pack \
        ...
        >> "$HOME/tmp/w12u2sel/pack.log" 2>&1 || { say "PACK FAIL $seed $ep"; continue; }
    "$REPO/scripts/m3a_sweep.sh" --bake "$pk" --bin "$DM" --grid full \
        --label "w12u2_${seed}_${ep}" --logdir "$HOME/tmp/w12u2sel" \
        --tsv "$HOME/tmp/w12u2sel/${seed}_${ep}.m3a_cells.tsv" > "$HOME/tmp/w12u2sel/${seed}_${ep}.kv" 2>>"$HOME/tmp/w12u2sel/m3a.log" || say "M3A FAIL $seed $ep"
    m3a=$(awk -F= '$1=="M3A_MEAN"{print $2; exit}' "$HOME/tmp/w12u2sel/${seed}_${ep}.kv" 2>/dev/null || echo NA)
    for spec in "cid22:$ROOT" "hfnlproxy:$VROOT"; do
      ...
      nice -n19 ionice -c3 "$REPO/target/release/bake_verdict" --bake "$pk" \
        ...
        --json "$HOME/tmp/w12u2sel/${seed}_${ep}.${ax}.json" > /dev/null 2>&1 || true
    done
    c22=$(python3 -c "import json;print(round(json.load(open('$HOME/tmp/w12u2sel/${seed}_${ep}.cid22.json'))['corpora'][0]['srocc'],4))" 2>/dev/null || echo NA)
    vhf=$(python3 -c "import json;print(round(json.load(open('$HOME/tmp/w12u2sel/${seed}_${ep}.hfnlproxy.json'))['corpora'][0]['srocc'],4))" 2>/dev/null || echo NA)
    echo -e "${seed}\t${ep}\t${m3a}\t${c22}\t${vhf}" >> "$OUT/w12u2_selection.tsv"
