...
# Source:        /mnt/v/output/zensim/csfw-g6-loo-2026-07-29/harness/p12_residual_boost_956.py
# sha256(source): e53c791fa4006cbacabd33797e5ed1ecba79e451796b23dc5e016b97778996aa
...
"""P12-style residual-boost of the csfw tier-1 12 lanes over the 944-MODEL
residuals — the bandvis wave's p12_residual_boost_944.py (itself a labeled
variant of the coordinator's scripts/v_next/p12_residual_boost.py) with two
labeled deviations, per the G6 LOO brief:

  1. Stage 1 = the FULL 944 streaming config (f0..f155 ++ f372..f943; the
     f156..f371 fold slots are structurally zero and self-mask via zero
     variance), NOT the 924 config — the brief asks for the marginal value of
     the 12 csfw columns over the *944-model* residuals. The stage-1 pool
     INCLUDES the unweighted GLOBAL_* twins and the whole append2 block, so
     stage 2 measures what the CSF WEIGHTING adds, not what global pooling
     adds.
  2. Stage 2 families = the three csfw lane families (W_GLOBAL_DMEAN,
     W_GLOBAL_CGAIN, W_GLOBAL_CLOSS; each 4 scale slots, Y-only,
     f = 944 + scale*3 + local) + the healthy-variance pair (DMEAN+CLOSS) +
     csfw_all. CGAIN is R-class rare-fire on SDR (std ~1e-5 documented) —
     its numbers are noise ratios, reported as structural.
...
D = Path("/mnt/v/zen/zensim-training/ext956-instrument-2026-07-29")
OUTDIR = Path("/mnt/v/output/zensim/csfw-g6-loo-2026-07-29")
OUT_CSV = OUTDIR / "p12_residual_boost_956_2026-07-29.csv"
OUT_MD = OUTDIR / "p12_residual_boost_956_2026-07-29.md"

N = 956
COLS_944 = list(range(0, 156)) + list(range(372, 944))
CS = 944
...
def fam_cols(slots: list[int]) -> list[int]:
    return [CS + s * 3 + k for s in range(4) for k in slots]
...
def main() -> None:
    print("stage 1: BVLS >=0 on the 944 config, ext_safesyn_full ...", flush=True)
    ...
    w944 = res.x
    active = int((w944[:-1] > 1e-9).sum())
    s1 = A @ w944
    print(f"  active={active}/{len(COLS_944)}  train R2={r2(ys, s1):.4f}  "
          f"SROCC={spearmanr(ys, s1).statistic:.4f}", flush=True)

    def stage1_pred(X: np.ndarray) -> np.ndarray:
        return np.hstack([(X[:, COLS_944] - mu) / sd, np.ones((len(X), 1))]) @ w944
    ...
    with open(OUT_MD, "w") as f:
        f.write("# P12-at-944 residual-boost of csfw tier-1 — 2026-07-29\n\n")
        f.write(f"Stage 1: BVLS >=0 on the FULL 944 config, ext_safesyn_full train "
                f"(active {active}/{len(COLS_944)}, train R2 {r2(ys, s1):.4f}) — "
                "labeled variant of p12_residual_boost.py (stage-1 = 944, incl. "
                "the unweighted GLOBAL twins + append2).\n"
                "Stage 2: per-csfw-family LINEAR on residuals. CGAIN is R-class "
                "rare-fire on SDR (std ~1e-5) — noise-ratio numbers, structural.\n"
        ...
        f.write("| rank | csfw family | mean CV-R2 |\n|---|---|---|\n")
