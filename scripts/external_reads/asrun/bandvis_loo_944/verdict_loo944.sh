...
# Source:        /mnt/v/output/zensim/bandvis-loo-2026-07-28/harness/verdict_loo944.sh
# sha256(source): d76bb15f5fcdeed4b7a25b368c012d1f1075abeb32c3e7e09e0cc626d04bda79
...
# Score lin944 + every LOO drop bake on the 8 canonical 944 legs via
# bake_verdict (tip build), features-root = the ext944 instrument dir.
# Corpora = the 8 of e2's suite that exist as canonical local legs (the
# imazen26/nonphoto NN-joined legs have no 944 extraction — reported as such).
set -u
...
run() {
  local name="$1" bin="$2"
  if [ -f "$OUT/$name.json" ]; then echo "skip $name (cached)"; return; fi
