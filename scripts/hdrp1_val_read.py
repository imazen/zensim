#!/usr/bin/env python3
"""Appendix Q step-1 in-domain read: forward a bake over the hdr944 val split
and report SROCC/PLCC vs the cvvdp-mix target (Q.3.1 — selection/sanity only;
never a UPIQ substitute).

Owners only: forward = `predict_features_with_bake` (wire format), stats =
`scripts/lib/zen_stats.panel` (canonical Rust panel). No stat math here.
...
ap.add_argument("--parquet",
                default="/mnt/v/output/zensim/hdr944-leg/hdr_v3mix944_valdigits_2026-08-03.parquet")
a = ap.parse_args()

t = pq.read_table(a.parquet)
fcols = [f"f{i}" for i in range(944)]
assert all(c in t.schema.names for c in fcols), "parquet lacks f0..f943"
