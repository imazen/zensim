#!/usr/bin/env python3
"""EXP-CROSS-CODEC-V6 multi-codec PJND score check (2026-05-19).

Mirrors eval_v4b_pjnd_check.py but globs cc4v6_*.bin and writes
v6_pjnd_check.md. Verifies V6 still satisfies the single-band T=63
cross-codec parity gate while range improves.

Usage:
    python3 scripts/v_next/eval_v6_pjnd_check.py /mnt/v/zen/zensim-eval/exp_cross_codec_v6_2026-05-19
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: eval_v6_pjnd_check.py <v6_dir>", file=sys.stderr)
        return 2
    ...
    out_md = v6_dir / "v6_pjnd_check.md"
    with open(out_md, "w") as f:
        f.write("# EXP-CROSS-CODEC-V6 multi-codec PJND score check\n\n")
        f.write("Target: each (source, codec) at PJND-q should score 63.0 ± a few units.\n")
        f.write("Gate: cross-codec score std per source ≤ 5.0.\n\n")
