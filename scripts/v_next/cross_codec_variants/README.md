...
The nine historical entry points `../run_cross_codec_v<N>_seed.sh` are thin shims
that `exec` the driver with the matching variant, so every command line quoted in
`benchmarks/*.md` and in `../run_v9_full_pipeline.sh` still works unchanged.
...
emitted, captured at commit `e9a705c0` by running the nine originals against a stub
trainer that printed its `"$@"` (sample args: seed 7; `W=0.5` for v2/v3/v4,
`anchor_w=0.05` for v4b, `anchor_w=1.0 anchor_p=0.30` for v6). `/mnt/v` is the data
root in every golden.
