#!/usr/bin/env python3
"""EXP-CROSS-CODEC-V7 multi-codec PJND score check (2026-05-19).

Mirrors eval_v4b_pjnd_check.py but globs cc4v7_*.bin and writes
v7_pjnd_check.md. Verifies V7 still satisfies the single-band T=63
cross-codec parity gate while range improves.

Usage:
    python3 scripts/v_next/eval_v7_pjnd_check.py /mnt/v/zen/zensim-eval/exp_cross_codec_v7_2026-05-19
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: eval_v7_pjnd_check.py <v7_dir>", file=sys.stderr)
        return 2
    ...
    out_md = v7_dir / "v7_pjnd_check.md"
    with open(out_md, "w") as f:
        f.write("# EXP-CROSS-CODEC-V7 multi-codec PJND score check\n\n")
