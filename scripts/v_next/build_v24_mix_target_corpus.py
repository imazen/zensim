...
def maybe_rename_feat_cols(t: pa.Table) -> pa.Table:
    """The LARGE corpus uses `feat_N` while the trainer expects `fN`.

    Rename in-place so the trainer's loader recognizes the feature
    columns. No-op when columns are already in the `fN` shape.
    """
