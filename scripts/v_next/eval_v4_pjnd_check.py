...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: eval_v4_pjnd_check.py <v4_dir>", file=sys.stderr)
        return 2
    v4_dir = Path(sys.argv[1])
    bakes = sorted(v4_dir.glob("cc4v4_*.bin"))
    if not bakes:
        print(f"no cc4v4_*.bin under {v4_dir}", file=sys.stderr)
        return 1
    ...
    # Write summary
    out_md = v4_dir / "v4_pjnd_check.md"
