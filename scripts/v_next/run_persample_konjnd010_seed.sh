#!/usr/bin/env bash
# V_24-persample-konjnd010 — per-sample α head with konjnd train_w=0.10.
# Compound hypothesis: per-sample α (won 5/0 vs scalar α) + gradient
# unstarvation (konjnd_w 0.02→0.10 gave +0.21 KonJND in earlier
# discriminator) may close the KonJND gap vs V_22-mix-LARGE.
#
# Args: <seed> <out_dir>
set -euo pipefail
SEED="${1:?seed}"
...
BAKE="$OUT_DIR/persample_konjnd010_seed${SEED}.bin"
LOG="$OUT_DIR/persample_konjnd010_seed${SEED}.log"
TRAINER_DIR=/home/lilith/work/zen/zensim--persample-konjnd010
TRAINER="$TRAINER_DIR/target/release/zensim_mlp_train"

# Mirror the V_22-LARGE / per-sample α recipe exactly except
# konjnd train weight: 0.02 → 0.10 (the only change).
"$TRAINER" \
