...
Reads:
  - {V4_DIR}/qsweep_v4.md → per-bake mono/tied/range/band-rmse
  ...
  - {V4_DIR}/v4_pjnd_check.md → cross-codec score std median

Emits a single combined table to {V4_DIR}/v4_summary.md.
...
def parse_qsweep(md_path: Path) -> dict[str, dict]:
    """Parse qsweep_v4.md, return per-bake {mono, tied, range, q5_med, q95_med}.
...
def parse_pjnd_check(md_path: Path) -> dict[str, dict]:
    """Parse v4_pjnd_check.md table for cc_std_median, agg_mean per bake."""
    out = {}
    if not md_path.exists():
        return {}
    text = md_path.read_text()
...
def main() -> int:
    ...
    qsweep = parse_qsweep(v4_dir / "qsweep_v4.md")
    t63 = parse_cross_codec_t63(v4_dir)
    verdicts = parse_verdicts(v4_dir)
    pjnd = parse_pjnd_check(v4_dir / "v4_pjnd_check.md")

    # Find all V4 bake names
    bakes = sorted({b.stem for b in v4_dir.glob("cc4v4_*.bin")})
    if not bakes:
        print("no cc4v4_*.bin found", file=sys.stderr)
        return 1

    out_md = v4_dir / "v4_summary.md"
