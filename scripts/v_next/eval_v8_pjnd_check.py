#!/usr/bin/env python3
"""EXP-CROSS-CODEC-V8 multi-codec PJND score check (2026-05-19).

Mirrors eval_v4b_pjnd_check.py but globs cc4v8_*.bin and writes
v8_pjnd_check.md. Verifies V6 still satisfies the single-band T=63
cross-codec parity gate while range improves.

Usage:
    python3 scripts/v_next/eval_v8_pjnd_check.py /mnt/v/zen/zensim-eval/exp_cross_codec_v6_2026-05-19
"""
...
def main() -> int:
    if len(sys.argv) < 2:
        print("usage: eval_v8_pjnd_check.py <v8_dir>", file=sys.stderr)
        return 2
    ...
    out_md = v8_dir / "v8_pjnd_check.md"
    with open(out_md, "w") as f:
        f.write("# EXP-CROSS-CODEC-V8 multi-codec PJND score check\n\n")
