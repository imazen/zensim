#!/usr/bin/env bash
# EXP-CROSS-CODEC-V6 evaluation harness (2026-05-19).
#
# Mirrors eval_cross_codec_v5.sh but globs cc4v6_*.bin in V6_DIR.
...
#
# Output: /mnt/v/zen/zensim-eval/exp_cross_codec_v6_2026-05-19/
set -euo pipefail
...
for bake in "${V6_DIR}"/cc4v6_*.bin; do
    [ -f "$bake" ] || continue
    name=$(basename "$bake" .bin)
    out_md="${V6_DIR}/verdicts/${name}.md"
...
python3 scripts/v_next/cross_codec_multi_band_check.py v6 "${V6_DIR}" || echo "v6 multi-band check failed"

echo
echo "All eval phases complete. See:"
echo "  ${RAW_OUT}                                       (qsweep mono/tied/range/band-rmse)"
echo "  ${V6_DIR}/verdicts/*.md                          (Mohammadi panel per bake)"
echo "  ${V6_DIR}/cross_codec_t63/*.tsv                  (cross-codec T=63 raw)"
echo "  ${V6_DIR}/v6_pjnd_check.md                       (V4-style PJND score std)"
echo "  ${V6_DIR}/v6_multi_band_check.md                 (multi-band parity gate)"
