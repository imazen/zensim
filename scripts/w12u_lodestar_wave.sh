...
HB=${W12U_HB:-$HOME/tmp/w12u/heartbeat}
mkdir -p "$(dirname "$HB")"
...
for seed in 4021 4022 4023; do
    ...
    if [ ! -f "$OUT/$stem.bin" ]; then
        ...
        nice -n19 ionice -c3 "$REPO/target/release/zensim_mlp_train" "${ARGV[@]}" \
            ...
            --group "tbig_hf_jw:/mnt/v/zen/zensim-training/sdr-pure-2026-08-28/tbig_hf_jw.parquet:1.0:0.0:both" \
            --seed "$seed" --out "$OUT/$stem.bin" \
            --dump-checkpoints-every 10 --dump-checkpoints-dir "$OUT/${stem}_ckpts" >> "$OUT/w12u_train.log" 2>&1 \
    ...
    if [ ! -f "$OUT/${stem}_packed.bin" ]; then
        ...
        nice -n19 ionice -c3 "$REPO/target/release/bake_dial_refit" pack \
            ...
            >> "$OUT/w12u_pack.log" 2>&1 || { say "PACK FAILED $stem"; FAILS=$((FAILS+1)); continue; }
    fi
    say "harvest ${stem}_packed"
    "$REPO/scripts/harvest_bakes.sh" --bake "$OUT/${stem}_packed.bin" --regime 944 \
        >> "$OUT/w12u_harvest.log" 2>&1 || { say "HARVEST FAILED ${stem}_packed"; FAILS=$((FAILS+1)); }
done
