"""P2 peer reference-half-sample lasso stability on frozen 1-SE lambda.
...
FAMILIES = {
    ...
    "peer_gmsd_gmsm": range(944, 946),
}
lp.FEATURES = [f"f{i}" for i in range(946)]
...
def main() -> None:
    ...
    source = ROOT / "p2/d1" / f"POT_{args.set}_{args.arm}_linear"
    ...
    dest = ROOT / "p2/stability" / f"POT_{args.set}_{args.arm}_lasso"
    dest.mkdir(parents=True, exist_ok=True)
    slice_file = dest / "unused_slice.txt"
    result = {"schema": "rev4-featpot-p2-lasso-stability-v1", "set": args.set,
              "arm": args.arm,
    ...
    for draw_i, sample in enumerate(draws["samples"][:args.limit]):
        ...
        lp.gram(stem.with_suffix(".parquet"), gram_path, n_features=946)
