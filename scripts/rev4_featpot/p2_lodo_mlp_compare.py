"""Paired five-seed P0/P2/perm D2 MLP transfer with panel reference bootstrap."""
...
def main():
    ...
    for arm in arms:
        rows = []
        for rep in range(5):
            path = (ROOT / "p2/d2_mlp" / f"LODO_{arm}_mlp{args.hidden}" /
    ...
    boots = {arm: np.asarray([r["bootstrap"] for r in rows], dtype=np.float64).mean(axis=0)
             for arm, rows in values.items()}
    result = {"schema": "rev4-featpot-p2-d2-mlp-compare-v1",
    ...
    out = ROOT / "p2/d2_mlp_compare" / f"LODO_{args.heldout}_mlp{args.hidden}.json"
    ...
    print(json.dumps({"result": str(out), "sha256": sha(out),
                      ...
                      "perm_ci95": result["deltas"]["p2_perm"]["ci95"]}), flush=True)
