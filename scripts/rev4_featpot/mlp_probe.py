"""One nested-CV H32/H128 MLP replicate for a POTENTIAL baseline cell.
...
def table(data: pd.DataFrame, indices: np.ndarray, bounds: tuple[float, float],
    ...
    # The rows came from the pinned Rev3 bank. Declare that producer identity
    # beside each derived training table so the trainer can check its registry
    # and slot coverage before it opens any target values.
...
def train(train_path: Path, val_path: Path | None, dest: Path,
    ...
    cmd += ["--target-column", "human_score", "--target-scale", "100",
            "--max-features", "944", "--hidden", str(hidden),
    ...
    if arm == "minus_basic":
        cmd += ["--keep-features", str(slice_file)]
    if dump:
...
def main() -> None:
    ...
    args = ap.parse_args()
    if (Path.home() / "tmp/zensim-paper/rev4/FLEET_FITS_GO.md").is_file():
        print("FLEET_GO: local P0 fit cell not started", flush=True)
        raise SystemExit(75)
    ...
    dest = ROOT / "fits" / f"POT_{args.set}_{args.arm}_mlp{args.hidden}" / f"{tag}_r{args.rep}"
    dest.mkdir(parents=True, exist_ok=True)
    ...
    result = {"schema": "rev4-featpot-mlp-replicate-v1", "label": "POTENTIAL — ceiling, not a model score",
