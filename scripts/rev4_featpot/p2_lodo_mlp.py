"""One D2 source-held-out MLP replicate for P0, P2 or matched permutation."""
...
from p2_mlp import TRAINER, predict, run, validation_scores
from linear_probe import BIN, panel_batch


def table_receipt(arm):
    path = ROOT / "p2/d2_mlp/tables" / arm / "receipt.json"
...
def train(arm, hidden, source_paths, validation, dest, init_seed, sample_seed, dump):
    ...
    cmd += ["--target-column", "human_score", "--target-scale", "100",
            "--max-features", "944" if arm == "r0" else "946",
    ...
    if arm != "r0":
        cmd += ["--historical-replay",
                "P2 D2 peer diagnostic: pinned Rev3 944 plus reviewed zenmetrics GMSD/GMSM; never ship"]
    if dump:
...
def main():
    ...
    dest = ROOT / "p2/d2_mlp" / f"LODO_{args.arm}_mlp{args.hidden}" / f"without_{args.heldout}_r{args.rep}"
    dest.mkdir(parents=True, exist_ok=True)
    inner_scores = []
    inner_receipts = []
    for validation in included:
        ...
        train(args.arm, args.hidden, training, (validation, paths[validation]),
              inner, init_seed, sample_seed, dump=False)
    ...
    train(args.arm, args.hidden, [(name, paths[name]) for name in included],
          None, final, init_seed, sample_seed, dump=True)
    ...
    if args.arm == "r0":
        data, label_meta = load_bank(eval_name, features=False)
    else:
        data, label_meta = load_peer(eval_name, args.arm, features=False)
    ...
    output = {"schema": "rev4-featpot-d2-mlp-replicate-v1", "label": "POTENTIAL — ceiling, not a model score",
