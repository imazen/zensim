"""One nested-CV H32/H128 MLP replicate for the reviewed P2 peer arm.
...
stay under /var/tmp/rev4-featpot/fits/POT_*. No board or auto-evaluation runs.
"""
...
TRAINER = ROOT / "target/debug/zensim_mlp_train"
...
def table(data: pd.DataFrame, indices: np.ndarray, bounds: tuple[float, float],
    ...
    # P2 is a diagnostic 946-column composite, not a registered pixel feature
    # producer: 944 pinned bank slots plus two reviewed peer columns. The
    # explicit historical-replay admission below records this unqualified
    # provenance; no P2 bake can be shipped as a production feature-set ID.
    Path(f"{path}.manifest.json").write_text(json.dumps({
        ...
        "peer_manifest_sha256": "8d2799c8a9ebaa3f25fa7518b3d7591ee7742b696d7f5592b9ace9974d0d27fc",
...
def train(train_path: Path, val_path: Path | None, dest: Path,
    ...
    cmd += ["--target-column", "human_score", "--target-scale", "100",
            "--max-features", "946", "--hidden", str(hidden),
            ...
            "--historical-replay", "P2 peer diagnostic: pinned Rev3 944 plus reviewed zenmetrics GMSD/GMSM; never ship",
...
def main() -> None:
    ...
    dest = ROOT / "p2" / "mlp" / f"POT_{args.set}_{args.arm}_mlp{args.hidden}" / f"{tag}_r{args.rep}"
    dest.mkdir(parents=True, exist_ok=True)
    ...
    result = {"schema": "rev4-featpot-p2-mlp-replicate-v1", "label": "POTENTIAL — ceiling, not a model score",
