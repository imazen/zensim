"""Paired reference-bootstrap P2/P0 and matched-permutation deltas via panel."""
...
def main():
    ...
    if args.scope == "d1":
        ...
        paths = {"p0": ROOT / "fits" / f"POT_{args.set}_r0_{args.model}" / "result.json",
                 "p2": ROOT / "p2/d1" / f"POT_{args.set}_p2_{args.model}" / "result.json",
                 "perm": ROOT / "p2/d1" / f"POT_{args.set}_p2_perm_{args.model}" / "result.json"}
        ...
        for phase in ("nested", "in_sample"):
            ...
            print(json.dumps({"set": args.set, "phase": phase,
                              "p2_delta": phases[phase]["deltas"]["p2"]["point_delta_srocc"],
                              "p2_ci95": phases[phase]["deltas"]["p2"]["ci95"],
                              "perm_ci95": phases[phase]["deltas"]["perm"]["ci95"]}), flush=True)
        output = ROOT / "p2/d1" / f"POT_{args.set}_{args.model}_compare.json"
        result = {"schema": "rev4-featpot-p2-compare-v1", "scope": "d1", "model": args.model,
        ...
        paths = {"p0": ROOT / "lodo" / f"LODO_r0_{args.model}" / "result.json",
                 "p2": ROOT / "p2/d2" / f"LODO_p2_{args.model}" / "result.json",
                 "perm": ROOT / "p2/d2" / f"LODO_p2_perm_{args.model}" / "result.json"}
        values = {key: json.loads(path.read_text()) for key, path in paths.items()}
        folds = {}
        for heldout in values["p0"]["folds"]:
            ...
            print(json.dumps({"heldout": heldout,
                              "p2_delta": folds[heldout]["deltas"]["p2"]["point_delta_srocc"],
                              "p2_ci95": folds[heldout]["deltas"]["p2"]["ci95"],
                              "perm_ci95": folds[heldout]["deltas"]["perm"]["ci95"]}), flush=True)
        output = ROOT / "p2/d2" / f"LODO_{args.model}_compare.json"
        result = {"schema": "rev4-featpot-p2-compare-v1", "scope": "d2", "model": args.model,
                  "B": 2000, "seed": 20260923, "unit": "reference",
