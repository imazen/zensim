#!/usr/bin/env bash
# POTENTIAL C1-C4: preregistered H32/H128 five-seed D1 grid and receipts.
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/../.."
export REV4_FIT_BIN=/var/tmp/rev4-featpot/main_build_20260924/target/debug/bake_dial_refit
export REV4_TRAINER_BIN=/var/tmp/rev4-featpot/main_build_20260924/target/debug/zensim_mlp_train
export ZEN_PANEL_BIN=/var/tmp/rev4-featpot/main_build_20260924/target/debug/panel
export OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1
root=/var/tmp/rev4-featpot/candidates

gate() {
    [[ -f $HOME/tmp/zensim-paper/rev4/PARTB_C1C4_DONE.md ]]
    [[ -f "$root/input_pin_committed.json" ]]
    [[ -f /var/tmp/rev4-featpot/main_build_20260924/binary_meta.json ]]
    [[ -x "$REV4_FIT_BIN" && -x "$REV4_TRAINER_BIN" && -x "$ZEN_PANEL_BIN" ]]
    [[ ! -e ${QUOTA_STOP_FILE:-$HOME/tmp/zensim-paper/rev4/CODEX_QUOTA_STOP.md} ]]
    local free_gb
    free_gb=$(df -BG --output=avail /home | tail -1 | tr -dc '0-9')
    (( free_gb >= 20 ))
}

for arm in c1 c4 c2 c3 all; do
    for variant in "$arm" "${arm}_perm"; do
        for set_name in kadid_train tid2013 konfig_train konjnd_bpg_train cid22_a25 aic3 kadid_select konfig_val; do
            for hidden in 32 128; do
                for outer in 0 1 2 3 4 full; do
                    for rep in 0 1 2 3 4; do
                        tag="o${outer}"
                        outer_args=(--outer "$outer")
                        if [[ "$outer" == full ]]; then
                            tag=full
                            outer_args=()
                        fi
                        dest="$root/mlp/POT_${set_name}_${variant}_mlp${hidden}/${tag}_r${rep}"
                        if [[ ! -f "$dest/result.json" ]]; then
                            gate
                            log="$root/log_mlp_${set_name}_${variant}_h${hidden}_${outer}_r${rep}.log"
                            printf 'START MLP %s %s H%s %s r%s %s\n' "$set_name" "$variant" "$hidden" "$outer" "$rep" "$(date -u +%FT%TZ)"
                            $HOME/tmp/devin/heavy --mem 16G --jobs 8 -- \
                                python scripts/rev4_featpot/candidate_mlp.py --set "$set_name" \
                                    --arm "$variant" --hidden "$hidden" \
                                    "${outer_args[@]}" --rep "$rep" > "$log" 2>&1
                            sha256sum "$dest/result.json" "$log"
                        fi
                        if [[ "$outer" != full && ! -f "$dest/importance.json" ]]; then
                            gate
                            log="$root/log_mlp_${set_name}_${variant}_h${hidden}_${outer}_r${rep}_importance.log"
                            $HOME/tmp/devin/heavy --mem 16G --jobs 8 -- \
                                python scripts/rev4_featpot/candidate_mlp_importance.py --set "$set_name" \
                                    --arm "$variant" --hidden "$hidden" \
                                    --outer "$outer" --rep "$rep" > "$log" 2>&1
                            sha256sum "$dest/importance.json" "$log"
                        fi
                        if [[ ! -f "$dest/ci.json" ]]; then
                            gate
                            log="$root/log_mlp_${set_name}_${variant}_h${hidden}_${outer}_r${rep}_ci.log"
                            $HOME/tmp/devin/heavy --mem 16G --jobs 8 -- \
                                python scripts/rev4_featpot/candidate_mlp_ci.py --set "$set_name" \
                                    --arm "$variant" --hidden "$hidden" \
                                    "${outer_args[@]}" --rep "$rep" > "$log" 2>&1
                            sha256sum "$dest/ci.json" "$log"
                        fi
                    done
                done
                out="$root/mlp_aggregate/POT_${set_name}_${variant}_mlp${hidden}/result.json"
                if [[ ! -f "$out" ]]; then
                    gate
                    log="$root/log_mlp_aggregate_${set_name}_${variant}_h${hidden}.log"
                    $HOME/tmp/devin/heavy --mem 16G --jobs 8 -- \
                        python scripts/rev4_featpot/candidate_mlp_aggregate.py --set "$set_name" \
                            --arm "$variant" --hidden "$hidden" > "$log" 2>&1
                    sha256sum "$out" "$log"
                fi
            done
        done
    done
done
touch "$root/mlp_d1_done"
printf 'C1-C4 MLP D1 grid complete %s\n' "$(date -u +%FT%TZ)"
