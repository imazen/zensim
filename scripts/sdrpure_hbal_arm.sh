#!/usr/bin/env bash
# H-BAL val-selection twin arm (registered in
# benchmarks/sdr_pure_retrain_wave_2026-08-28.md "H-BAL ARM").
# Purity argv + the tbig HF leg VAL-WEIGHTED ONLY (selection, not gradient).
set -uo pipefail
...
HB=${HBAL_HB:-$HOME/tmp/hbal/heartbeat}
mkdir -p "$(dirname "$HB")"
...
for seed in 4003 4004 4005; do
    ...
    if [ ! -f "$OUT/$stem.bin" ]; then
        ...
        nice -n19 ionice -c3 "$REPO/target/release/zensim_mlp_train" "${ARGV[@]}" \
            --group "tbig_hf:/mnt/v/zen/zensim-training/sdr-pure-2026-08-28/tbig_hf_pure.parquet:0.0:1.0:both" \
            --seed "$seed" --out "$OUT/$stem.bin" >> "$OUT/hbal_train.log" 2>&1 \
    ...
    if [ ! -f "$OUT/${stem}_packed.bin" ]; then
        ...
        nice -n19 ionice -c3 "$REPO/target/release/bake_dial_refit" pack \
            ...
            >> "$OUT/hbal_pack.log" 2>&1 || { say "PACK FAILED $stem"; FAILS=$((FAILS+1)); continue; }
    fi
    say "harvest ${stem}_packed"
    "$REPO/scripts/harvest_bakes.sh" --bake "$OUT/${stem}_packed.bin" --regime 944 \
        >> "$OUT/hbal_harvest.log" 2>&1 || { say "HARVEST FAILED ${stem}_packed"; FAILS=$((FAILS+1)); }
done
