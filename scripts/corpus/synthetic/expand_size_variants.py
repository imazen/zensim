...
"""
# ----------------------------------------------------------------------
# IMPORTANT — per-crop audit finding (2026-05-26)
#
# The 2026-05-26 per-crop contribution audit
# (~/work/zen/_ml-inventory-2026-05-20/16-per-crop-contribution-audit-2026-05-26.md)
# DEFINITIVELY SHOWED:
#
# (a) Axis-inverted (rotation) crop pairs are REDUNDANT:
#     - 769x513  ↔ 513x769:  mean cosine dist 0.037, target Pearson 0.968
#     - 1022x818 ↔ 818x1022: mean cosine dist 0.032, target Pearson 0.974
#     Generating both rotations is wasted compute and wasted storage.
#     Pick one orientation per aspect ratio.
#
# (b) Mod-8 boundary crops are NULL EFFECT:
#     - boundary-vs-clean delta on mix_target: mean -0.20 ± 0.78
#     - No codec shows the predicted block-edge stress
#     The original rationale (test codec block-boundary handling via
#     mod-8 ±1/-2 dimensions) is data-falsified. 769x513 / 1022x818
#     behave like 1024sq does. Drop them.
#
# (c) Scale (512 ↔ 1024) is the ONLY axis that carries signal:
#     - mean cosine dist 0.105 — 3x the rotation-pair distance.
#     - _512sq is undervalued, not redundant; smaller crops expose
#       distinct codec failure modes.
#
# RECOMMENDED USAGE going forward: generate ONLY 512sq + 1024sq. The
# 4-bucket boundary expansion (769x513, 513x769, 1022x818, 818x1022)
# this script produces by default is retained for backward compatibility
# with the historical safesyn corpus but should NOT be used for new
# synthetic content.
# ----------------------------------------------------------------------
