...
| `D` (**FAST SDR** — behind the default-on `candidate-profiles`) | 372-declared-width linear, 28 nonzero coefficients over `f0..156` only, + id100+negrich dial spline. Reads no pool slot, so the `f156..372` pass is skipped: **2.5×/4.4×/3.5× faster than the full 944-class walk at 1/8/16 threads, −21 % peak RSS** | **0.8634** | 4.2 KB ADD156 id100-negrich |
