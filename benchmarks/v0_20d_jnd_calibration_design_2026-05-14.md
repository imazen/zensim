...
## Concrete next step (this tick)

1. **DONE (this doc)**: empirical AIC-3 JND analysis — 76.71 vs
   KonJND's 63 documented.
2. **NEXT**: write `zensim-bench/examples/aic3_jnd_sanity.rs` that
   loads V0_18 ship, runs it across the 300 AIC-3 Anchor stimuli,
   and reports the median zensim score at AIC-3 distortion ∈
   [0.9, 1.1] with a ±5 tolerance gate. Skip if zensim
   doesn't match SSIMULACRA2 closely at JND (within ±5 of 76.71).
3. **LATER**: V0_20d Option B recalibration (queued behind the
   dial-semantic-flip coordination work).
