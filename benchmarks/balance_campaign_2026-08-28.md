...
| **score coverage (the honest-stop gate)** | verified FIRST, at the CELL level via `encode_sha` (a blob count is not a cell count — `brnat` dedups 22.3 %): **brnat 7,488/7,488 = 100.00 %**, **brsdr 4,971/4,979 = 99.84 %**; 8 cells (0.16 % of brsdr, 0.06 % of the wave) carried as `metrics_present=None`, never dropped. Scorer live throughout (5 containers; blobs 2,706 → 2,721 during the analysis). |
