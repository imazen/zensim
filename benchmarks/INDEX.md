...
> **★★ `freeze_check --select` WAS BLIND TO THE G-ADDR DIAL-ADDRESSABILITY CONTRACT, 2026-09-06:**
> [`select_gaddr_prefilter_2026-09-06.md`](select_gaddr_prefilter_2026-09-06.md). The registered
> k-seed selection rule's PRIMARY (profile floor count) and TIE-BREAK (`selection_composite`) were
> both completely blind to `dial.addressability` — measured on the best-of-all wave picking
> `A_plain` (G-ADDR CONTRACT 4/6, fails **C5+C6**) over five arms at 6/6, because neither key can
> see a contract failure. **Fixed at the owner**: a candidate (or seed group with one member) that
> MEASURES a CONTRACT-tier fail is now an absolute selectability veto — dial addressability is a
> HARD ship gate (user rule 2026-09-04), not a floor a high CID22 can outvote — and `A7r`'s
> per-codec floor-representability folds into `--floor-basis all`'s count. New `--floor-basis
> legacy` reproduces the pre-fix rule byte-for-byte (audit only). Failing-first test verified to
> fail with the veto stubbed out; 39/39 green. Re-run over the best-of-all wave (demonstration:
> pick flips `A_plain` → `F_nonneg32`) and the 125-cell VERIFIED-FAIR board (9 lower-ranked
> recipes flip to NOT SELECTABLE; current #1 unaffected — already contract-clean). Registry:
> `select-rule-blind-to-dial-contract-2026-09-06` → superseded by
> `select-rule-contract-prefilter-fixed-2026-09-06`.
...
[`corruption_head_serving_2026-09-06.md`](corruption_head_serving_2026-09-06.md) — the serving half of [`corruption_head_theories_2026-09-06.md`](corruption_head_theories_2026-09-06.md), pre-registered at [`../docs/PLAN_CORRHEAD_SERVING_2026-09-06.md`](../docs/PLAN_CORRHEAD_SERVING_2026-09-06.md) and pushed before any code existed. **The tree head can now be served, and every claim is a gate rather than an argument.** The format is its **own `ZCTH` v1 magic, NOT a ZNPR metadata blob** — `zenpredict` is frozen at the `zenanalyze-api` contract, and independently every consumer holding a `zenpredict::Model` dispatches through `Predictor::predict`, so a tree behind a plausible identity layer is silently mis-scored by anything that does not know to look (the `--regime 944` defect shape); a distinct magic makes head-vs-dial confusion a refusal at byte 0. What ZNPR got right is copied, not re-invented: magic + `u16` version + `u64` schema hash over the canonical shape + a `(offset, len)` section table + a declared-feature-id list, so a head obeys the same dense contract as `zensim::declared_feature_ids`. **Gates, all PASS:** the Rust evaluator reproduces sklearn's `decision_function` at **0 ulp** over 33,591 test + 2,016 gate rows (the tree walk is exact arithmetic, so 0 was the bar, not a tolerance) and the calibrated probability to **max |Δ| 3.330669e-16** — all of it `exp`, and it moves **no** deadband decision on any of 35,607 rows (fire set identical, 19,308 + 662 firing); adopting the one shared `gate_score` composition leaves `bake_verdict --full-json` **BYTE-IDENTICAL** on the incumbent logistic head (`e5dab5d1…`); and end-to-end through `bake_verdict` the tree head reproduces the theory lane's gate-grid row **exactly** — DEPLOY `pass_q20` **0.9985119047619048** (671 of 672 = the record's 99.85 %), head `pass_q20` 1.0, D's dial alone 0.26785714285714285 — which also **settles by measurement** that the theory lane's `rank_break` tie-break is a no-op at the `T = 0.9` deadband (the Rust path does not apply it and lands on the same 671). **One wrong reading of sklearn's own source, corrected:** `IsotonicRegression._build_f` builds a `scipy.interpolate.interp1d(kind="linear")`, so the obvious move is to reimplement `interp1d._call_linear` (leftmost bracket, convex combination) — **that is not the code that runs**; `interp1d.__init__` routes plain `linear` to `_call_linear_np`, i.e. `np.interp`, **rightmost** bracket and **slope** form. MEASURED on a real 90-knot fit over 25,092 queries the `np.interp` form is **bit-identical** to `iso.predict` while the scipy-source form differs by up to **1.11e-16**, located exactly on the plateau edges an isotonic fit is made of. *Reading the constructor is not reading the call.* **The owner extension is additive and PROVEN so** — re-running the incumbent `d228` recipe through the patched `train_corruption_head.py` reproduced the SHIPPED artifacts byte-for-byte (`da411c8c…`, `a7ad4e85…`, identical `split.tsv`); the `hgb` bake is byte-identical across the determinism lane's BLAS thread pin (it never went through the lbfgs solve), while the logistic one moves to `6f97b653…` — exactly the "1 thread" value that lane registered, reproduced independently. `t6` now shares one `fit_head` with the exporter and **re-running it reproduces both its TSVs byte-for-byte**, so the refactor is arithmetic-neutral by measurement. **Servability, both halves:** `check_servable_by` refuses a head reading a slot the profile's plan does not populate (negative control: a head declaring `f300` is REFUSED with the slot named), and — the availability half, measured — `ZensimProfile::D` emits 372 features with the peaks block **`f156..228` populated 72/72** while zeroing only `f228..371`, which is what makes a 372-wide head attachable to D at all. **Cost:** the tree forward is **659 ns/compare** against Profile D's own forward at **1.05 µs** (0.63×, 95 % CI [−41.1 %, −32.0 %], zenbench 1T) ≈ **0.011 % of a 576² compare** — and the **INCUMBENT logistic head costs 1.76 µs, 2.7× the 6,100-node tree and more than the dial it guards**, so the nonlinear head is not a speed tradeoff. (The in-situ `ssim2_speed_bar` arm was extended to sniff both formats but **degenerated to 3 rounds at CV 51–141 %** on a shared box and reported a "+10.3 ms marginal" for a sub-microsecond forward — recorded as the measurement hazard it is, and the reason `corrhead_forward.rs` exists.) **Nothing shipped:** no weights, no profile, no board cell, no default-path behaviour, and `cargo public-api` on a default build is **zero delta** (1284 items); the runtime companion (`Zensim::with_corruption_head` / `corruption_verdict`) is `#[doc(hidden)]` behind the non-default `corruption-head` feature, off `compute`'s path, and its proposed public surface awaits the user's approval in the plan doc §3. Gates: `scripts/verify_corrhead_serving.sh` + 18 in-crate tests. Ledger `docs/DATASET_HISTORY.md` §3.49 (Ledger ROUND 101).

**The board's operative dial ruler becomes the LADDER instrument (2026-09-06)**:

[`board_ladder_ruler_2026-09-06.md`](board_ladder_ruler_2026-09-06.md) — pre-registered at [`../docs/PLAN_BOARD_LADDER_RULER_2026-09-06.md`](../docs/PLAN_BOARD_LADDER_RULER_2026-09-06.md) and pushed (`22ffc5d2`) before any cell was re-graded. The board's `dial.addressability` column was cut on the 2026-05-29 CANONICAL grid while every current gate is defined on the 2026-09-05 FLOOR-DENSE LADDER, and `--graft-gaddr`'s same-grid gate correctly REFUSED to merge the two — so the ladder readings lived in a sidecar the board could not see and `freeze_check --select`'s CONTRACT veto could not act on them. **Resolved by giving the ladder reading its own block, not by relaxing the refusal.** **450 of 508** cells now carry `dial_ladder`; the badge, the `floors ok` (A7r) column, the G-ADDR panel and the `--select` veto all read it through ONE owner, and `dial.addressability` is **byte-untouched** (0 of 508 changed) and surfaced as hover-only `dial_canonical`. **The era gate is MEASURED, not assumed**: the ladder-944 grid populates 905 slots with `f156..371` LIVE (slot-set sha8 `b6811ae0`) while `bake_verdict`'s default 944 grid populates 689 with that block STRUCTURALLY ZERO (`026c0aba`), so a bake that READS it would be scored through weights that never took a gradient — 34 cells are refused on that ground, 359 are admitted because `block_profile` proves their `f156..371` weights exactly zero, 22 are already pools-era. All **58** NOT-MEASURED cells carry a recorded reason (+21 off-width, 2 peer metrics with no reference table, 1 bake in a deleted sibling workspace). **Probe recovery is PROVEN, not guessed** — `measured.negtail` is a function of `(bake, probe)` only and never touches the dial grid, so the right probe is the one whose read reproduces the board block bit-for-bit (verified identical on `W10L9PH_s4007_packed`); 14 recovered, 0 unrecovered. Two traps found on the way: `measured.identity` mixes probe reads with GRID counts (`n_grid_cells_total` 4,817 vs 9,593) and can never match across instruments, and this build emits C5/C6 as measured CHECKS while serialising no `measured.identity` dict. **What flips:** the NOT-SHIPPABLE badge goes **63 → 75**, but **21 of the +22 are FIRST-EVER coverage** — the true ruler effect is **ONE up-flip against TEN down-flips (−9)**, and on the 93 cells shared with the 2026-09-04 as-run set the contract-fail count goes 46 → 42, reproducing gate doc §17.7's 47 → 43 on a different code path. **The switch is DIRECTIONAL PER ROW, not uniformly stricter**: C1 (monotonicity) gets harder because the ladder samples q 0..30 at step 1, C2 (ceiling-clamp share) gets easier because floor-dense sampling puts fewer cells near the clamp (`W10L9PH_s4007_packed` 0.0908 → 0.0306 against 0.05). **`freeze_check --select`'s pick does NOT move** (`11e243eb0b86` = `fc2_372_S228_H128_s4004/5/6` under all three `--gaddr-block` modes; `canonical` byte-identical to a pristine `main@origin` binary), but **10 fair cells go `NO — CONTRACT FAIL` → `yes`** — the veto's removal-only property holds *within* a ruler, not *across* one. **⚠ The board's A7r fractions are `resolvable`, NOT the `distinct` ones the ladder record's §9 tables carry**, and the difference flips a verdict: under `distinct` shipped Profile D fails jpeg by one ladder, under the operative rule **the ADD156/D lineage passes all five codecs** (0.6667 / 1.0000 / 0.6667 / 1.0000 / 1.0000) and is the only bake family on the board that does — `peer_ssim2` is the only other five-codec pass and `peer_butteraugli` fails four of five. Board-wide **10 of 450 clear all five, 293 clear none**, and every best-of-all / fast-class arm collapses on `avif-rav1e` (0.13–0.21 against a 0.641 bar). Tooling: `scripts/gaddr_board_ladder.py {grade|graft}`, `promote_fulleval.py --graft-gaddr-ladder` (mirror gate: proves a REGISTERED ladder grid where `--graft-gaddr` proves the board's own; self-test with a negative control), `freeze_check --gaddr-block auto|canonical|ladder`. All 9 acceptance gates PASS; the fair board's margin under the 12 MiB cap fell to **53,984 B** because 279 cells gained a first-ever G-ADDR block. Three append-only registry entries scope every pre-switch dial number to the canonical instrument. Gate doc §19; ledger `docs/DATASET_HISTORY.md` §3.50 (Ledger ROUND 102).
