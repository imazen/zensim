...
> **★★ THE HYBRID + the exam's 8-THREAD amendment 2026-09-01:** [`hybrid_candidate_2026-09-01.md`](hybrid_candidate_2026-09-01.md) — **APPENDIX B** amends the ssim2 exam's **W4** to bind at **BOTH 1 and 8 threads** (user directive), with the opponent given its `rayon` feature and each candidate priced on **its own extraction regime plus its own forwards** (the exam gave the two flagships and `Q7b` one row although the flagships are blind to f156-371 and `Q7b` uses 107 of those columns). Then the hybrid: a convex score-space blend of `W10L9PH_s4004_packed` and the W-LIN 7b winner `Q7b`, on ONE keyed pools-944 substrate — **it PASSES W1, which neither parent passes**, over a measured window **w ∈ [0.76, 0.86]** bounded BELOW by LIVE and ABOVE by KonJND. **KonJND is SUPER-ADDITIVE** (both parents below ssim2 at 0.5006/0.5118; the blend peaks at **0.5390** vs ssim2's 0.5272). **W3 passes and the blend repairs both parents' ladder pathologies** — Q7b's deepest q≥85 backwards step 91.3 → 30.4 (below ssim2's own 52.2), the flagship's 0.0376 tied rate → 0.0000. **W2 still FAILS, structurally**: the named near-lossless win survives to w ≈ 0.4–0.6 and W1 does not start until 0.76, so the two feasible regions are **disjoint** — the model holding the only non-circular win over ssim2 is the one 0.147 behind on LIVE. New instruments: `bake_verdict --ensemble-weights` (convex, three bit-identical endpoint gates), the `add156_156basic` / `flagship_944off` / `q7b_944pools` / `hybrid_944pools` amended-W4 bench arms, and **LIVE + AIC-4 added to the paired bootstrap** — the exam's exclusion was ROOT-scoped (372 root max |Δ| 1.12; both 944 roots 0.0).
