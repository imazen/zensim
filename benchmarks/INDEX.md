...
**G-ADDR negative tail — the −50 per-codec re-pin (2026-09-05)**:

[`dial_addressability_gate_2026-09-04.md`](dial_addressability_gate_2026-09-04.md) **§16** — the USER RULING that retires `A7`/`A8`/`A9` (`peer_ssim2`'s incidental depth on one probe, plus a `frac<0 ≥ 1.0000` bar that was **definitional** rather than measured) and replaces them with `A7r`/`A8r`/`A9r` at an absolute **−50**, graded **PER CODEC** because *"codecs are all different, some go lower than others"* — and that is a MEASUREMENT: on the canonical grid `avif` reaches −55.3545 and `webp` −51.8466 and are graded, while `jpeg` (−8.0450) and `jxl` (−39.6858) never do and are **EXEMPT**, since asking a dial to go deeper than the truth bars it for being correct. `A8r` is the one pooled row because the negtail probes carry no codec column at all (their rows are KADIS distortion types, and grading them per *distortion* family at −50 fails every bake ever built on one n=8 family); `A9r` is **report-only on a new `Tier::Report`** that gates nothing, with its 0.90 registered `user-provisional` and never read as a bar. Re-graded 6 scorers × 2 eras × 2 pin sets and all 97 fair board cells: **D-peaks' sole blocker `A8` becomes `A8r` PASS**, but it now fails `A7r` on **webp by 4.28** and shipped D by **1.88** — a named codec with a measured gap instead of an arbitrary depth — and 76 of 97 board cells fail the pooled `A8r` the retired pins read as NOT MEASURED. The contract-driven **NOT SHIPPABLE badge is unchanged (47 measured / 46 on board, 0 contract rows changed), asserted by the grafter and by a unit test rather than assumed**, and `--gaddr-tail-pins retired` reproduces every pre-ruling number. New committed owner: [`scripts/gaddr_board_regrade.sh`](../scripts/gaddr_board_regrade.sh) — the board re-grade, which reconstructs each cell's invocation from the 2026-09-04 as-run logs so it is provably the same run with one flag changed.
