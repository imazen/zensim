...
- The producer of `ratios.tsv` and `xyb8.tsv` is committed at `zensim/src/gmsbank_calibration_instrument.rs`; `scripts/gmsbank/calibration_instrument.sh` compiles and runs it explicitly. The runner requires a fresh directory containing `planes.tsv` and its decoded RGB8 files. Missing arguments fail and existing output files are never overwritten.
...
- The original ratio/XYB producer was recovered from `/var/tmp/gmsbank/stale-rescue-20260923T2357Z/worktree.tar` and committed as the tool above. The explicit tool replayed all 652 pairs in a fresh directory, exit 0. Its `ratios.tsv` SHA256 `767197dfae605bb05eb078fa27b064dc456cbcb7a1e645531f0ee30fb1338c40` and `xyb8.tsv` SHA256 `5898620078ecc4c64e7f50bcaa5637f399fa1c80ed546cea0bce3e0a3fd53376` match the original outputs byte for byte. UTC, cwd and exact command are in `/var/tmp/gmsbank/command_records/correction_calibration_replay.json`.

## Limit

The selected TRAIN bank contains only medium and large images under the preregistered size cutoffs. **20 of 32 strata are empty**, including every tiny and small stratum, so the workspace size-sweep rule for source constants is unmet. The content and corpus coverage above is reported without inventing replacements or changing thresholds. The calibration measures unit conversion; it does not establish a human-quality benefit.
