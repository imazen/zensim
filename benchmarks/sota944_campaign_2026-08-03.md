...
| **band-cut duplicate census** (all flagged, none silently changed) | THREE other implementations exist besides the owner, and the census is stated in full so the next pass is not surprised: (1) `scripts/v_next/blend_lib.py::BANDS` — deciles on an AUTO-STRETCHED axis, so its `B9` is "top tenth of the observed range", not "target ≥ 0.90"; re-pointing it changes what a live dashboard's columns mean, which is a decision, not a refactor. Now carries a pointed comment + "do not add callers". (2) `scripts/baseline_panels_2026-05-18/extract_panels.py::per_band_panel` — a verbatim copy of the OLD fixed-decile logic, self-described as "mirroring bake_verdict.rs", in a dated May-2026 baseline dir; it now mirrors nothing. (3) `scripts/v_next/v0_20_low_n_band_analysis.py` parses `### <corpus> 10-band SROCC` out of a verdict markdown — a header `bake_verdict` has not emitted for a long time, so that parser was **already dead before this pass** (verified: the emitter said `10-band full Mohammadi panel`). Both (2) and (3) are May-era artifacts; neither is on a live path, and deleting them is a separate call |
