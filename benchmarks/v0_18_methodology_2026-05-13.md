...
cargo run --release -p zensim-validate --bin zensim_mlp_train -- \
  ...
  --hidden 128 --epochs 300 --pairs-per-epoch 50000 --lr 0.001 --seed 1 \
  --tv-pairs-file /tmp/zensim_loop/tv_pairs_with_bands.tsv \
  --tv-weight 1.0 \
...
The TV pairs TSV was generated by `scripts/v_next/regen_tv_pairs.py
--emit-bands` from the same training CSVs.
...
Construction code lived in `/tmp/zensim_loop/concat_construct.py`
during cycle-14; the canonical reproduction script is
`scripts/v_next/concat_three_way.py` (added in tick 645 — verify it
matches the on-disk V0_17 by hash before relying on it).
