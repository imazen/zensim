...
**One defect found by that scan — FIXED IN THE OWNER, not worked around.**
`ext_tid`'s canonical TSV named
`/mnt/v/dataset/tid2013/reference_images_png/I25.png`; the corpus holds
**`i25.png`** (lowercase — TID2013's own inconsistency; the source BMP is
`i25.bmp` too), so 120 of 3,000 rows named a path that does not exist.

**Root cause, in the owner:**
`scripts/canonical_corpus/build_fr_corpus_pairs.py::build_tid()` resolved the
DISTORTED side case-insensitively (its docstring even says so) but built the
REFERENCE name as `bmp.split("_")[0].upper() + ".png"` — forcing upper case.
Every row of the one reference TID ships lowercase was then dropped by a
`miss += 1; continue`, and the builder printed the count and exited 0.

**Fixed 2026-08-30, three changes:**
1. `build_tid()` resolves BOTH sides through `_case_insensitive_index()`.
2. A shared `_require_all_resolved()` makes any unresolved label row **fatal**
   in the builders (`FRPAIRS_ALLOW_MISSING=1` downgrades to a warning — the
   decision then belongs to the caller and is visible in the invocation, the
   only skip form the project allows).
3. `v2_ab_extract` no longer writes a short table: if `rows.len() !=
   pairs.len()` it prints every SKIP reason, **refuses to write the partial
   CSV**, and exits 3 (`ZENSIM_AB_ALLOW_MISSING=1` is the caller's opt-out).
   Verified both directions — rc=3 with no file written, and the opt-out
   writing with a loud warning.

The canonical TSV was regenerated by the fixed builder (3,000/3,000 rows, 0
skipped); the previous file is preserved as
`tid_pairs_ab.tsv.pre-i25case-2026-08-30.bak` and a normalized diff confirms
**only the 120 `I25`→`i25` reference paths changed** (240 diff lines = 120 rows
× 2), every other row byte-equal. No corpus file was renamed or created.
**TID was then re-extracted at pools-944 AND at the control regime from the
canonical TSV**: 3,000/3,000 rows, all 25 references present with `I25`
carrying its full 120 rows, **G-P1 728/728 bit-identical**, **G-E
`ref_basename` + `human_score` EQUAL to the stored root**, orientation
**+1.000000**. The interim `ZM944_PAIRS_<LEG>` override is retained in the
driver as a general capability but is no longer used for TID.

Every other leg scanned clean: **0 missing refs, 0 missing distorted files
across the other 146,195 pairs.**
