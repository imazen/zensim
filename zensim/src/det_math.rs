...
//! | score mapping (`metric.rs` `powf(b)`, `exp`) | `powf`, `exp` | **NOT routed, and it IS an exposure** — the exponents (0.5979, 1.2244, 0.6130, and the profile-supplied `b`) are not powers of two, so the SCORE is libc-dependent by the same mechanism. Named in the source record's §5 as unmeasured; still unmeasured. It is not a FEATURE, so no stored table moves, but two boxes can disagree on a dial value in the last ulp |
//!
