...
### Balanced trail v2 (`PreviewV0_5BalancedV2`, task #176)

**Audience.** Same as the Balanced trail — general-purpose ranking +
KADID/TID/KonJND best-in-class coverage — PLUS callers that need
**dial-honest user-facing semantics**: typing "score 60" lands at
JND (PJND anchor), typing "score 30" lands at JOD, and the dial
spans the full [0, 100] range. Replaces the Balanced ship's broken
`clamp(0, 100)` behavior on the distance-shaped raw output without
retraining the network.

**Mechanism.** Same V_22-mix-LARGE+iwssim bake bytes as
[`PreviewV0_5Balanced`] (300 → 128 → 1 i8 + zerobias + lz4 MLP).
The only addition is a 7-knot **post-network PCHIP spline**
calibration baked into the new `zentrain.output_calibration_spline`
metadata key. The spline is fit at calibration time on the V9
extended-range anchor parquet:
...
at fit time — the underlying Balanced network puts target=10 in
raw output *above* target=0 (a direction-violation that the
monotone spline cannot fix). The spline linearly extrapolates the
target=10 region using the surrounding knots' slope.

**Gate** (extends Balanced trail's gate with V9 anchor-landing
sub-gates, no retraining required):

1. **SROCC preserved** within ±0.01 vs Balanced base on every
...
The 21-unit error on target=10 is the cost of the network's
...
**V2 vs base Balanced — what changes**:

- Raw output of the Balanced bake is distance-shaped (high raw =
  low quality, range ≈ [-23, 38] across the V9 anchor parquet).
- Production `apply_mlp_scoring` applied `clamp(0, 100)` to the
  distance-shaped raw output, pinning **96.8 % of CID22
  predictions to 0** (the `bake_post=clamp` SROCC was 0.28 vs
  raw 0.83 — the dial was structurally broken).
- The spline maps the distance-shaped raw onto the dial-honest
  [0, 100] score scale via PCHIP, so the post-`clamp` SROCC now
  matches the raw SROCC.

**No retraining** — only the metadata changes. The underlying
network is the SAME bake bytes (`v22_mix_cv40_konjnd_002_LARGE_iwssim_2026-05-18.bin`
md5 `b703c9cfc7e1908faf5b0e78dc823221`). The calibrated bake is
41,766 bytes (+71 over the 41,695 base — the spline adds 36 bytes
uncompressed; the lz4-packed delta is +71).

See `benchmarks/v_balanced_v2_2026-05-20_methodology.md` for the
...
| V11-SUBSTRATE-V2-CLEAN (median s3) | 2026-05-20 | 372 | per-sample-α head dispatch | 0.8754 | 0.7943 | 0.9196 | 0.8921 | 0.4627 | FAIL (B>>A on KonJND −0.43 vs Balanced ship) | FAIL ON ALL 3 GATE CRITERIA — vs V_24-per-sample-α s4 (`v_compression_persample_2026-05-18.bin`, the actual Compression ship) on identical `2026-05-15-full-features` parquet root: **step 1 FAIL** (CID22 ΔSROCC +0.0113 under +0.015 decisive cut + PWRC +0.0082 under +0.010 + Z-RMSE −0.027 under −0.030 → promising-A only, no decisive A>>B); **step 2 FAIL** (AIC-3 ΔSROCC −0.0240 + PWRC −0.0163 + Z-RMSE +0.026 → decisive B>>A on other compression corpus); **step 3 FAIL** (KonJND ΔSROCC −0.3453 vs −0.10 cap = 3.45× over, triangulated PWRC −0.2490 + Z-RMSE +0.410). 5-seed CI on KonJND: 0.29–0.46 across all seeds — collapse is structural, not seed-dependent. V11 substrate (multi-codec ssim2-anchored anchor parquet + V_24 per-sample-α head + V_24 hparams) is a CID22 specialist that drops AIC-3 within-codec rank discrimination AND breaks the PJND anchor by construction (no PJND-anchored term in the training mix). Per `benchmarks/v11_compression_falsification_2026-05-20.md`. Task #191. |
| V11-A'-372 v4 CLEAN (median s2) | 2026-05-20 | 372 | per-sample-α head dispatch | 0.8944 | 0.8173 | 0.9253 | 0.8903 | 0.3942 | FAIL (KonJND −0.499 vs Balanced ship) | FAIL — KonJND drift −0.369 vs Compression ship far exceeds −0.10 cap. CID22 +0.030 decisive; AIC-3 tied; KADID −0.006/TID +0.008 within tolerance. The single blocker is KonJND. 372-feat decoder fix (commit `3bd88eca`) added zenavif+zenjxl path-deps to the 372-feat extractor, recovering 55,200 previously-skipped cells. Net 372-feat IW-pool contribution: +0.022 CID22, +0.045 KonJND over 300-feat V11 v2 clean — but neither rescues the KonJND collapse. Per `benchmarks/v11_a_372_v4_methodology_2026-05-20.md` and `benchmarks/v11_a_372_falsification_2026-05-20.md`. Task #195. |
| V11-A-CC-EQ-WEIGHT-SWEEP w ∈ {0.05, 0.10, 0.20, 0.50} (5 seeds × 4 tiers, median by CID22) | 2026-05-20 | 372 | per-sample-α head dispatch | 0.8932–0.8965 across 4 tiers | 0.8137–0.8207 | 0.9244–0.9291 | 0.8875–0.8932 | **0.3875–0.4312** across 4 tiers (mean 0.40) | FAIL all 4 tiers (KonJND drift −0.461 to −0.505 vs Balanced) | FAIL all 4 tiers (KonJND drift −0.377 to −0.421 vs Compression). Cross-codec-eq mechanism is structurally KonJND-incompatible at every weight tier ≥ 0.05. The hypothesis that "at w << 1.0 the rank-preserve term dominates so KonJND survives" is FALSIFIED — KonJND collapses identically at w=0.05 (0.3925), w=0.10 (0.3916), w=0.20 (0.3875), w=0.50 (0.4312), w=1.00 (0.3942). The collapse is binary (mechanism gates whether the gradient is applied), not magnitude-dependent. Cross-codec consistency (butter_p3 at JND ≈ 1.0, JOD ≈ 2.2-2.4) is essentially constant across all w — the mechanism IS effective at cross-codec convergence; the KonJND price is paid in full regardless of w. NO ship at any tier. Closes the cross-codec-eq frontier at this substrate. Per `benchmarks/v11_cc_eq_weight_sweep_falsification_2026-05-20.md`. Task #197. |
