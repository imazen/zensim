...
//! ```
//!
//! `ZENSIM_ROOT_FORM=libm` reproduces the shipped (revision-1) arm and
//! `=sqrt` the deterministic one, so the SAME pair of binaries measures both
//! sides of the era.
//!
...
fn emit(tag: &str, r: &[[u8; 3]], d: &[[u8; 3]], w: usize, h: usize) {
    ...
    let f = out.features();
    // The score is dumped too: `metric.rs`'s raw-distance -> score mapping
    // calls `powf` with non-power-of-two exponents, so it is a SECOND libc
    // exposure that this crate has not fixed. Dumping it makes that exposure
...
fn main() {
    eprintln!("root_form_env={:?}", std::env::var("ZENSIM_ROOT_FORM").ok());
