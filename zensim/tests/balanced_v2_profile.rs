//! Smoke test for `PreviewV0_5BalancedV2` runtime wiring.
//!
//! Task #176 (2026-05-20) ships the Balanced bake
//! (`v22_mix_cv40_konjnd_002_LARGE_iwssim_2026-05-18.bin`) extended
//! with a **post-network monotone PCHIP spline calibration** in the
//! new `zentrain.output_calibration_spline` metadata key. The
//! re-baked bytes ship at `zensim/weights/v_balanced_v2_2026-05-20.bin`;
//! this test guards the `PreviewV0_5BalancedV2` variant +
...
//!      `zentrain.output_calibration_spline` metadata on a STANDARD
//!      MLP (no per_sample_alpha_head, no hybrid_head, no
//!      tanh_output_head) without panicking.
//!   4. The bake bytes differ from PreviewV0_5Balanced (the underlying
//!      MLP weights are the same, but the spline metadata produces a
...
//!     persisted at `benchmarks/v_balanced_v2_2026-05-20_verdict.md`.
//!   * JND/JOD anchor landing (60.000 / 30.000 exact) — verified at
//!     calibration time, persisted at the calibration log.
//!   * Cross-codec consistency — V9-style smoke demo. The Balanced
...
fn balanced_v2_score_in_range() {
    ...
    assert!(
        score.is_finite(),
        "balanced-v2 score is NaN/Inf: {score} \
         (PCHIP-spline runtime dispatch on a plain MLP may be broken)"
    );
    assert!(
...
fn balanced_v2_score_in_range_across_distortion_levels() {
    ...
    for delta in (0..50).step_by(5) {
        ...
        assert!(
            ...
            "balanced-v2 score out of range or NaN at delta={delta}: {score}"
        );
    }
}

#[test]
fn balanced_v2_differs_from_balanced_base() {
    // PreviewV0_5BalancedV2 wraps the same underlying MLP as
    // PreviewV0_5Balanced + a `zentrain.output_calibration_spline`
    // metadata entry. The Balanced base's raw output is distance-shaped
    // (high raw = low quality) and the production `clamp(0, 100)` was
    // pinning high-quality predictions to 0. The spline maps the
    // distance-shaped output onto the dial-honest [0, 100] scale, so
    // the V2 score MUST differ from the base on any non-trivial pair.
    let (src, dst) = make_pair_with_delta(64, 64, 16);
    let s = RgbSlice::new(&src, 64, 64);
    let d = RgbSlice::new(&dst, 64, 64);
    ...
    assert!(
        ...
        "non-finite score(s): balanced-v2={s_v2} balanced-base={s_base}"
    );
    assert!(
        (s_v2 - s_base).abs() > 0.01,
        "balanced-v2 produced identical score to balanced-base \
         (v2={s_v2}, base={s_base}); include_bytes! may point at the wrong file \
         or the PCHIP spline runtime dispatch is missing for plain MLPs"
    );
}

#[test]
fn balanced_v2_identity_short_circuit_preserved() {
    // The V0_5 identity-image short-circuit (fix at metric.rs:1634) must
    // still produce score=100 on byte-identical pairs even with the
    // spline metadata in the bake (the short-circuit fires BEFORE
    // apply_mlp_scoring, so this is a regression check on the
    ...
    assert_eq!(
        r.score(),
        100.0,
        "balanced-v2 identity short-circuit broken: identical pair scored {}",
        r.score()
    );
}
