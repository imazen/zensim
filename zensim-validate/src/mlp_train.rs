...
mod tests {
    ...
    // ----------------------------------------------------------------
    // EX-1 (2026-05-17): Thurstone Case V Gaussian-CDF NLL tests.
    // ----------------------------------------------------------------

    #[test]
    fn norm_cdf_anchors() {
        // Standard normal CDF reference points.
        assert!((norm_cdf(0.0) - 0.5).abs() < 1e-7);
        // Φ(0.6745) = 0.75 (Thurstone JND anchor — d = Φ⁻¹(0.75)).
        assert!(
            (norm_cdf(0.6745) - 0.75).abs() < 1e-4,
            "Φ(d) = {}",
            norm_cdf(0.6745)
        );
        // Φ(1.96) ≈ 0.975.
        assert!((norm_cdf(1.96) - 0.975).abs() < 1e-3);
        // Tails clamp into strictly (0, 1) so log Φ never explodes.
        assert!(norm_cdf(-50.0) > 0.0);
        assert!(norm_cdf(50.0) < 1.0);
    }

    #[test]
    fn norm_pdf_anchors() {
        // φ(0) = 1/√(2π) ≈ 0.39894.
        assert!((norm_pdf(0.0) - 0.398_942_280_4).abs() < 1e-7);
        // φ(1) = 1/√(2π) · exp(-1/2) ≈ 0.24197.
        assert!((norm_pdf(1.0) - 0.241_970_724_5).abs() < 1e-7);
        assert!((norm_pdf(-1.0) - 0.241_970_724_5).abs() < 1e-7);
    }

    #[test]
    fn thurstone_loss_correct_ordering_low() {
        // target = +1, pred_diff (= yb - ya) large positive ⇒ a is
        // ranked below b in distance, ordering CORRECT ⇒ Φ(u) ≈ 1 ⇒
        // loss ≈ 0 and gradient ≈ 0.
        let (loss, dl) = thurstone_pair_loss_and_grad(1.0, 5.0, 0.6745);
        assert!(loss < 0.01, "loss={loss}");
        assert!(dl.abs() < 0.01, "dl={dl}");
    }

    #[test]
    fn thurstone_loss_wrong_ordering_pushes_correctly() {
        // target = +1, pred_diff large negative ⇒ a ranks ABOVE b in
        // distance — wrong direction. Loss must be large positive and
        // gradient must push pred_diff UP (∂L/∂pred_diff < 0 because
        // we want to INCREASE pred_diff).
        let (loss, dl) = thurstone_pair_loss_and_grad(1.0, -3.0, 0.6745);
        assert!(loss > 1.0, "wrong-direction loss should be large; got {loss}");
        assert!(
            dl < 0.0,
            "gradient must pull pred_diff up; got dl/d(pred_diff)={dl}"
        );
    }

    #[test]
    fn thurstone_loss_target_negative_mirrors() {
        // target = -1 swaps the desired direction. pred_diff negative
        // is now CORRECT ⇒ loss ≈ 0; pred_diff positive is wrong.
        let (loss_correct, _) = thurstone_pair_loss_and_grad(-1.0, -3.0, 0.6745);
        let (loss_wrong, dl_wrong) = thurstone_pair_loss_and_grad(-1.0, 3.0, 0.6745);
        assert!(loss_correct < 0.1, "correct-direction loss should be small; {loss_correct}");
        assert!(
            loss_wrong > 1.0,
            "wrong-direction loss should be large; got {loss_wrong}"
        );
        // ∂L/∂pred_diff for target=-1 wrong direction should push DOWN.
        assert!(
            dl_wrong > 0.0,
            "for target=-1 wrong, gradient must pull pred_diff down; dl={dl_wrong}"
        );
    }

    #[test]
    fn thurstone_numerical_gradient_check() {
        // Compare analytical ∂L/∂pred_diff against a centred finite
        // difference. Use a non-trivial point.
        let d = 0.6745;
        let target = 1.0;
        let pred_diff = -0.4;
        let eps = 1e-6;
        let (_, dl) = thurstone_pair_loss_and_grad(target, pred_diff, d);
        let (l_plus, _) = thurstone_pair_loss_and_grad(target, pred_diff + eps, d);
        let (l_minus, _) = thurstone_pair_loss_and_grad(target, pred_diff - eps, d);
        let num = (l_plus - l_minus) / (2.0 * eps);
        assert!(
            (dl - num).abs() < 1e-4,
            "analytical {dl} vs numerical {num}"
        );
    }
    ...
    fn thurstone_jnd_at_unit_gap() {
        ...
        // by construction.
        let (_, dl_correct) = thurstone_pair_loss_and_grad(1.0, 1.0, 0.6745);
        // Gradient at u = +0.6745 should still pull a bit (since
        // there's a 25 % residual error mass), not zero.
        assert!(dl_correct < 0.0, "gradient at 1-JND correct still pulls; got {dl_correct}");
        // Loss = -ln(0.75) ≈ 0.288.
        let (loss, _) = thurstone_pair_loss_and_grad(1.0, 1.0, 0.6745);
        let expected = -(0.75_f64.ln());
        assert!(
            (loss - expected).abs() < 1e-4,
            "loss should equal -ln(0.75)≈0.2877; got {loss}"
        );
    }

    #[test]
    fn thurstone_train_smoke_runs_and_recovers_ranking() {
        // End-to-end: train a tiny MLP on a synthetic group with the
        // Thurstone loss and confirm SROCC > 0.85. Mirrors the
        // RankNet smoke test below.
        let n_features = 4;
        let n = 50;
        let mut rng = SplitMix64::new(7);
        let features_owned: Vec<Vec<f64>> = (0..n)
            .map(|_| (0..n_features).map(|_| rng.next_normal()).collect())
            .collect();
        let targets: Vec<f64> = features_owned
            .iter()
            .map(|f| 50.0 + 20.0 * f[0] + 10.0 * f[1].tanh())
            .collect();
        let feat_refs: Vec<&[f64]> = features_owned.iter().map(|v| v.as_slice()).collect();
        ...
        assert!(
            srocc > 0.85,
            "Thurstone MLP failed to recover synthetic ranking: SROCC={srocc:.4}"
        );
    }
