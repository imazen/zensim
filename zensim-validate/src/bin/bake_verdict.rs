...
fn score_row(
    ...
    match result {
        Ok(out) => {
            // Per-sample α head dispatch — most general; runs first.
                ...
                per_sample_alpha_head
            {
                ...
                return (alpha * y_rank + (1.0 - alpha) * y_pool, alpha);
            }
            ...
            if let Some((rw, rb, p_norm)) = pool_head_reducer {
                ...
                let y = out.first().copied().map(|v| v as f64).unwrap_or(f64::NAN);
                (y, f64::NAN)
            }
        }
...
/// [reducer_w[0..4]] [reducer_b] [p_norm]` — total bytes = 4·(2·n_hidden + 8).
...
/// Per-sample α head dispatch tuple — used to thread W_α + b_α +
/// rank_w + rank_b + reducer_w + reducer_b + p_norm through the
/// per-corpus scoring chain.
...
fn render_corpus(
    ...
    let scores: Vec<f64> = g
        ...
        .map(|row| {
            ...
            if alpha.is_finite() && per_sample_alpha_head.is_some() {
                alpha_samples.push(alpha);
            }
            y
        })
        .collect();
    // Per-sample α diagnostic: emit α distribution stats for this corpus.
    if let Some(_) = per_sample_alpha_head
        && !alpha_samples.is_empty()
    {
        let n = alpha_samples.len() as f64;
        let mean = alpha_samples.iter().sum::<f64>() / n;
        let mut sorted = alpha_samples.clone();
        ...
        eprintln!(
            "  α({}): n={} mean={:.3} p05={:.3} p25={:.3} p50={:.3} p75={:.3} p95={:.3}",
            corpus.display,
            sorted.len(),
            mean,
            p(0.05),
...
fn main() -> ExitCode {
    ...
    // Priority: per_sample > hybrid > pool. A bake carries at most one.
    ...
    eprintln!(
        "bake: n_inputs={n_inputs}  feature_transforms={}  pool_head={}  hybrid_head={}  per_sample_α={}",
        if has_transforms { "yes" } else { "no" },
        if pool_head_reducer.is_some() { "yes" } else { "no" },
        if hybrid_head.is_some() { "yes" } else { "no" },
        if per_sample_alpha_head.is_some() { "yes" } else { "no" },
    );
    ...
    {
        let w_alpha_mag = w_alpha.iter().map(|&v| (v as f64).abs()).sum::<f64>()
            / w_alpha.len().max(1) as f64;
        eprintln!(
            "  per_sample_α: |W_α|={} mean|W_α|={:.4} b_α={:+.4} (init-α={:.3}) |rank_w|={} rank_b={:.4} reducer_w=[μ={:.4} σ={:.4} max={:.4} p6={:.4}] reducer_b={:.4} p_norm={:.2}",
            w_alpha.len(),
            w_alpha_mag,
            b_alpha,
            1.0 / (1.0 + (-(*b_alpha as f64)).exp()),
            rank_w.len(),
            rank_b,
            reducer_w[0], reducer_w[1], reducer_w[2], reducer_w[3], reducer_b, p_norm
        );
